...
class DataEngine:
    ...
    def filter_message(self, symbol, message):
        print(f"{bcolors.RED}Filtering {symbol} for {message}.{bcolors.RESET}")
    ...
    def calculate_volatility(self, stock_price_data):
        ...
        close_prices = [float(item[CLOSE_PRICE_INDEX])
                        for item in stock_price_data_list]
            ...
            try:
                ...
                if not not_found:
                    ...
                    if volatility < self.VOLATILITY_THRESHOLD:
                        self.filter_message(
                            symbol, f"low volatilty {volatility} < {self.VOLATILITY_THRESHOLD}")
                        continue
                    ...
                    self.features_dictionary_for_all_symbols[symbol] = {
                        "features": features_dictionary, "current_prices": stock_price_data, "future_prices": future_prices}
                    ...
                    if np.isnan(feature_list).any() == True:
                        self.filter_message(symbol, "nan in feature_list")
                        continue
                    ...
                    if average_volume_last_30_tickers < self.VOLUME_FILTER:
                        self.filter_message(
                            symbol, f"low volume {average_volume_last_30_tickers} < {self.VOLUME_FILTER}")
                        continue
